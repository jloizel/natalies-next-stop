...
const EditPost = () => {
  ...
  const addNestedContentBlock = (subIndex: number, blockIndex: number) => {
    ...
    if (block) {
        ...
        // Add a new nested block of type list
        block.nestedBlocks.push({
            type: 'list',
            content: '', // Initialize content for nested block
            subContent: [], // Initialize subContent for nested blocks
        });
    }
  ...
  // Handle change for nested content block
  const handleNestedContentChange = (subIndex: number, blockIndex: number, nestedIndex: number, value: string) => {
    const updatedSubsections = [...post.subsections];
    const block = updatedSubsections[subIndex].contentBlocks[blockIndex];

    if (block && block.subContent) {
      block.subContent[nestedIndex] = value; // Update the nested content
    }
                  ...
                  {block.type === 'list' ? (
                    <div>
                        <textarea
                            ...
                            onChange={(e) => handleContentBlockChange(index, blockIndex, 'subContent', e.target.value)}
                            className={styles.textarea}
                        />
                        <ul>
                            {block.subContent?.map((item, itemIndex) => (
                                <li key={itemIndex}>{item}</li> // Render as bullet points
                            ))}
                        </ul>
                        <button type="button" onClick={() => addNestedContentBlock(index, blockIndex)} className={styles.button}>
                            Add Nested Content
                        </button>
                        {/* Render nested content inputs */}
                        {block.subContent && block.subContent.map((subContent, nestedIndex) => (
                            <div key={nestedIndex} className={styles.nestedContent}>
                                <input
                                    type="text"
                                    placeholder="Nested Content"
                                    value={subContent}
                                    onChange={(e) => handleNestedContentChange(index, blockIndex, nestedIndex, e.target.value)}
                                    className={styles.input}
                                />
                            </div>
                        ))}
                  ...
                  {block.type === 'image' && (
                    <div>
                      <input
                        ...
                        onChange={(e) => handleContentBlockChange(index, blockIndex, 'images', e.target.value)}
                        className={styles.input}
                      />
                    </div>
                  )}
