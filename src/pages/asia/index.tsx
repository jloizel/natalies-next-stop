...
const AsiaPage = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        const data = await getPostsByContinent('Asia'); // Fetch posts for Asia
        console.log('Fetched posts for Asia:', data); // Log to confirm data structure
        setPosts(data);
      } catch (err) {
  ...
  const handleCountryClick = (country: string) => {
    router.push(`/asia/${country.toLowerCase()}`); // Convert country to lowercase for URL
  };
  ...
  const handlePostClick = (country: string, postId: string) => {
    router.push(`/asia/${country.toLowerCase()}/${postId}`); // Convert country to lowercase for URL
  };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.headerImageContainer}>
        <img className={styles.headerImage} src="/images/asia.jpg" />
        ...
        {loading ? (
          ...
          <>
            {uniqueCountries.length > 0 ? ( 
              ...
              <div className={styles.errorContainer}>Please check back later for Asia travel blogs</div> 
            )}
          </>
        )}
      </div>
      <div className={styles.latestPostsContainer}>
        {posts.length > 0 && <div className={styles.latestPostsHeader}>Latest Asia Blogs</div>}
