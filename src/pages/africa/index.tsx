...
const AfricaPage = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      setError(''); 
      try {
        const data = await getPostsByContinent('Africa'); 
        setPosts(data);
      } catch (err) {
  ...
  const handleCountryClick = (country: string) => {
    router.push(`/africa/${formatCountryForURL(country).toLowerCase()}`); 
  };

  // Handle post click to navigate to post page
  const handlePostClick = (country: string, postId: string) => {
    router.push(`/africa/${formatCountryForURL(country)}/${postId}`); 
  };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.headerImageContainer}>
        <img className={styles.headerImage} src="/images/africa.jpg" />
