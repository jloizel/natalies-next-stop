...
import styles from './centralamerica.module.css'; // Create a CSS module for styling
...
const CentralAmericaPage = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        const data = await getPostsByContinent('CentralAmerica'); // Fetch posts for CentralAmerica
        console.log('Fetched posts for CentralAmerica:', data); // Log to confirm data structure
        setPosts(data);
      } catch (err) {
  ...
  const handleCountryClick = (country: string) => {
    router.push(`/centralamerica/${country.toLowerCase()}`); // Convert country to lowercase for URL
  };
  ...
  const handlePostClick = (country: string, postId: string) => {
    router.push(`/centralamerica/${country.toLowerCase()}/${postId}`); // Convert country to lowercase for URL
  };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.headerImageContainer}>
        <img className={styles.headerImage} src="/images/centralamerica.jpg" />
        ...
        {loading ? (
          ...
          <>
            {uniqueCountries.length > 0 ? ( // Check if there are unique countries
              ...
              <p>No travel blogs available for Central America.</p> // Message when there are no posts
            )}
          </>
        )}
      </div>
      <div className={styles.latestPostsContainer}>
        {posts.length > 0 && <div className={styles.latestPostsHeader}>Latest Central America Blogs</div>}
