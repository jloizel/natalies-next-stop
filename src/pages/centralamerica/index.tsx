...
const CentralAmericaPage = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      setError(''); 
      try {
        const data = await getPostsByContinent('Central America'); 
        setPosts(data);
      } catch (err) {
  ...
  const handleCountryClick = (country: string) => {
    router.push(`/centralamerica/${formatCountryForURL(country).toLowerCase()}`); 
  };

  // Handle post click to navigate to post page
  const handlePostClick = (country: string, postId: string) => {
    router.push(`/centralamerica/${formatCountryForURL(country)}/${postId}`); 
  };
        ...
        {loading ? (
          ...
          <>
            {uniqueCountries.length > 0 ? ( 
              ...
              <div className={styles.errorContainer}>Please check back later for Central America travel blogs</div> 
            )}
          </>
        )}
      </div>
